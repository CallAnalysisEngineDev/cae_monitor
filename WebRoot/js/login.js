...
var Login = (function() {
	...
	function askForPublickey() {
		...
		//data中指明这个type就代表该请求是用来获取服务器的公钥的
		};
		//如果获取成功,则进入login_2函数
	...
	function login_2(data) {
		...
		if (encrypt.encrypt_MD5(data.publicKey) == data.summary) {
			...
			//使用3DES对管理员账号进行加密
			var encryptAdminUserAccount = encrypt
					.encrypt_3DES($("#useraccount").val());
			//使用3DES对管理员密码进行加密
			var encryptAdminUserPassword = encrypt.encrypt_3DES($("#password")
					.val());
			...
			var encryptCheckEncode = encrypt.encrypt_MD5(JSON
					.stringify(formData));
